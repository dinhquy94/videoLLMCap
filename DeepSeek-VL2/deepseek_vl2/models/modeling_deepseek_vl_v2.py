...
class DeepseekVLV2ForCausalLM(DeepseekVLV2PreTrainedModel):
    ...
    def prepare_inputs_embeds(
        ...
        Args:
            ...
            video (torch.FloatTensor): [b, n, dim]
        ...
        if images is None or images_spatial_crop.sum() == 0:
            ...
            # b, _ = input_ids.shape
            video_index = 0
            for i in range(input_ids.shape[0]): # Lặp qua từng batch
                indexes = torch.where(input_ids[i] == video_token_id)
                for index in indexes:
                    embeddings[i][index] = video[i][video_index]
                    video_index += 1
        ...
        print("input_embeds input_embeds", input_embeds)
